#!/usr/bin/env python3
"""
Analyze previous-generation models experiment results
"""
...
def main():
    base_dir = Path("data/previous_gen_experiment")
    seeds = [42, 43, 44]
    ...
    print("="*70)
    print("PREVIOUS-GENERATION MODELS EXPERIMENT RESULTS")
    print("="*70)
    print(f"Models: {len(models)} (GPT-4o, DeepSeek V2.5)")
    ...
    print("**System Optimization Potential:**")
    print()
    for model_name, res in sorted_models:
        ...
        if "SYSTEM" in verdict:
            print(f"  🔧 **{model_name}**: {gap_pct:+.1f}% improvement → System tuning helps!")
        elif "MODEL" in verdict:
            print(f"  🤖 **{model_name}**: {gap_pct:+.1f}% degradation → Need better base model")
        else:
            print(f"  ⚖️  **{model_name}**: {gap_pct:+.1f}% change → Already well-calibrated")
